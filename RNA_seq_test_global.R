...
Gene_expression=read.csv("MMRF_processed.csv",header=T,sep=",",row.names=1)
train_data<-as.data.frame(scale(Gene_expression[-c(ncol(Gene_expression))]))
...
MMRF<-train_data
MMRF<-MMRF[MMRF$label!="CENSORED",]
MMRF$label<-as.factor(MMRF$label)
MMRF_test<-MMRF[,!(colnames(MMRF)%in%c("Study"))]
colnames(MMRF_test)<-gsub("X","",colnames(MMRF_test))
...
levels(ensemble_rf_pred)<-levels(ensemble_test$label)
ensemble_rf_accuracy<-Accuracy(ensemble_rf_pred,ensemble_test$label)
levels(ensemble_nn_pred)<-levels(ensemble_test$label)
