...
setwd("/lclhome/hguan003/MM")
Gene_expression=read.csv("/biorg/data/Myeloma/GSE24080UAMSentrezIDlevel.csv",header=T,sep=",",row.names=1)
clinical<-read.csv("/lclhome/hguan003/MM/globalClinTraining.csv",header=T,sep=",",row.names=2,stringsAsFactors = FALSE)
...
efit<-eBayes(fit)
limma_gene<-toptable(efit,coef=2,number=10000,p.value=0.05)
write.csv(limma_gene,file="limma_gene.csv")
