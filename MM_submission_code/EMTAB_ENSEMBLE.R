...
train_data<-train_data[,as.logical(colSums(train_data!=0))]
model.entrezs<-colnames(train_data)[1:(ncol(train_data)-1)]
...
test_MM$label<-NULL
test_MM$Age<-Age_ISS[match(rownames(test_MM),rownames(Age_ISS)),1]
train_MM$Age<-Age_ISS[match(rownames(train_MM),rownames(Age_ISS)),1]
...
test_MM$ISS<-Age_ISS[match(rownames(test_MM),rownames(Age_ISS)),2]
train_MM$ISS<-Age_ISS[match(rownames(train_MM),rownames(Age_ISS)),2]
