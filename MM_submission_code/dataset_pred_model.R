...
##load individual selected gene lists
...
Gene_expression=read.csv("UAMS_training_balanced.csv",header=T,sep=",",row.names=1)
...
E_genes_list<-rownames(Gene_expression)[-c(1,2)]
model.entrezs<-c(eu_genes_list,E_genes_list)
model.entrezs<-unique(model.entrezs)

colnames(Gene_expression)<-gsub("X","E",colnames(Gene_expression))
Age_ISS<-Gene_expression[1:2,]
colnames(Age_ISS)<-colnames(Gene_expression)
Age_ISS<-as.data.frame(t(Age_ISS))
Age_ISS$`888888888`<-Age_ISS$`888888888`/100
...
U_ISS<-Gene_expression[2,]
colnames(U_ISS)<-colnames(Gene_expression)
U_ISS<-as.data.frame(t(U_ISS))
...
H_ISS<-as.data.frame(t(H_ISS))

U_ISS<-rbind(H_ISS,U_ISS)
...
for (i in  1: nrow(pred_stacked)) {
  if (pred_stacked[i]=="X1") {
      input<-test_MM[i,]
      input$ISS<-U_ISS[match(rownames(input),rownames(U_ISS)),]
    ...
    input<-test_MM[i,]
    input$Age<-Age_ISS[match(rownames(input),rownames(Age_ISS)),1]
    input$ISS<-Age_ISS[match(rownames(input),rownames(Age_ISS)),2]
...
#save all of trained models with genes lists in .Rd file
save(U_svmfit,U_rffit,U_nn_fit,U_lvq_fit,U_gbm_fit,U_glmfit,U_fit_stacked,
     ...
     U_genes_eu_list,M_genes_eu_list,
     M_mean,M_sd,U_mean,U_sd,
