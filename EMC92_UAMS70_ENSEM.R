...
UAMS_eu<-subset(train_data,rownames(train_data)%in%U_train_MM)
UAMS_eu_test<-subset(train_data,rownames(train_data)%in%U_test_MM)
...
EMTAB_eu<-subset(train_data,rownames(train_data)%in%E_train_MM)
EMTAB_eu_test<-subset(train_data,rownames(train_data)%in%E_test_MM)
...
HOVON_eu<-subset(train_data,rownames(train_data)%in%H_train_MM)
HOVON_eu_test<-subset(train_data,rownames(train_data)%in%H_test_MM)
...
MMRF_eu<-subset(train_data,rownames(train_data)%in%M_train_MM)
MMRF_eu_test<-subset(train_data,rownames(train_data)%in%M_test_MM)
...
train_MM<-train_eu
train_label<-train_MM$label
train_MM$label<-NULL
test_MM<-test_eu
test_label<-test_MM$label
test_MM$label<-NULL
...
a<-myimpute(t(train_MM),clus=20)
train_MM<-t(a)
train_MM<-as.data.frame(train_MM)
a<-myimpute(t(test_MM),clus=20)
test_MM<-t(a)
test_MM<-as.data.frame(test_MM)

eu_mean<-rowMeans(as.matrix(t(train_MM)))
eu_sd<-rowSds(as.matrix(t(train_MM)))


saveRDS(eu_mean,"./eu_mean.rds")
saveRDS(eu_sd,"./eu_sd.rds")

train=sweep(train_MM,MARGIN=2,eu_mean,"-")
train_MM<-sweep(train,MARGIN=2,eu_sd,"/")
test=sweep(test_MM,MARGIN=2,eu_mean,"-")
test_MM<-sweep(test,MARGIN=2,eu_sd,"/")
train_MM$label<-train_label
test_MM$label<-test_label
...
svmtrain<-train_MM
svmfit<-svm(as.factor(label)~.,data=svmtrain,gamma=0.01,tune.control=tc,kernel = 'radial',probability=TRUE)
svm_pred_train<-predict(svmfit,svmtrain[,1:(ncol(svmtrain)-1)])
prob<-predict(svmfit,test_MM[,1:ncol(test_MM)],probability=TRUE)
pred<-predict(svmfit,test_MM[,1:ncol(test_MM)])
pred<-as.vector(pred)
...
prob_rf<-predict(rffit,test_MM[,1:ncol(test_MM)],type='prob')
th<-0.5
pred_rf<-factor(ifelse(prob_rf$X1>th,"1","-1"))
rf_prob_train<-predict(rffit,RFtrain[,1:(ncol(RFtrain)-1)],type="prob")
...
nn_fit<-caret::train(label~.,
                     ...
                     maxit=100,
                     trace=FALSE)
pred_nn<-predict(nn_fit,test_MM[,1:ncol(test_MM)])
nn_pred_train<-predict(nn_fit,RFtrain[,1:(ncol(train_MM)-1)])
prob_nn<-predict(nn_fit,test_MM,type='prob')
nn_prob_train<-predict(nn_fit,RFtrain[,1:(ncol(train_MM)-1)],type="prob")
nn_pred_train<-predict(nn_fit,RFtrain[,1:(ncol(train_MM)-1)])
#prob_nn<-predict(nn_fit,test_MM,type='prob')
...
lvq_grid <- expand.grid(size=c(5,10,20,50,60,80,100,120,140), k=c(1,2,4,5,10))
LVQtrain<-train_MM
lvq_fit<-caret::train(label~.,
                      ...
                      tuneGrid=lvq_grid,
                      preProcess="scale")
pred_lvq<-predict(lvq_fit,test_MM[,1:ncol(test_MM)])
lvq_pred_train<-predict(lvq_fit,RFtrain[,1:(ncol(train_MM)-1)])
...
gbm_fit<-caret::train(label~.,
                      ...
                      verbose=FALSE,
                      trControl=fitcontrol)
prob_gbm<-predict(gbm_fit,test_MM[,1:ncol(test_MM)],type='prob')
th<-0.5
pred_gbm<-factor(ifelse(prob_gbm$X1>th,"1","-1"))
gbm_prob_train<-predict(gbm_fit,RFtrain[,1:(ncol(train_MM)-1)],type="prob")
...
glmfit<-caret::train(label~.,
                     ...
                     preProcess=NULL)
prob_glm<-predict(glmfit,test_MM[,1:ncol(test_MM)],type='prob')
th<-0.5
pred_glm<-factor(ifelse(prob_glm$X1>th,"1","-1"))
glm_prob_train<-predict(glmfit,RFtrain[,1:(ncol(train_MM)-1)],type="prob")
...
write.csv(class_result,file="EU_HR_FLAG.csv")
write.csv(ae_predict,file="EU_accuracy.csv")

#save model seperately
saveRDS(svmfit,"./eu_svmfit.rds")
saveRDS(rffit,"./eu_rffit.rds")
saveRDS(nn_fit,"./eu_nn_fit.rds")
saveRDS(lvq_fit,"./eu_lvq_fit.rds")
saveRDS(gbm_fit,"./eu_gbm_fit.rds")
saveRDS(glmfit,"./eu_glmfit.rds")
saveRDS(fit_stacked,"./eu_fit_stacked.rds")


rownames(test_MM)<-gsub("X","eu",rownames(test_MM))
write.csv(rownames(test_MM),"eu_test_MM.csv")
