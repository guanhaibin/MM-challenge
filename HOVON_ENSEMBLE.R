...
test_MM$label<-NULL
H_mean<-rowMeans(as.matrix(t(train_MM)))
H_sd<-rowSds(as.matrix(t(train_MM)))
train=sweep(train_MM,MARGIN=2,H_mean,"-")
train_MM<-sweep(train,MARGIN=2,H_sd,"/")
test=sweep(test_MM,MARGIN=2,H_mean,"-")
test_MM<-sweep(test,MARGIN=2,H_sd,"/")
train_MM$label<-train_MM_label
test_MM$label<-test_MM_label
nfea<-ncol(train_MM)-1

saveRDS(H_mean,"./H_mean.rds")
saveRDS(H_sd,"./H_sd.rds")
...
write.csv(class_result,file="HOVON_TRAIN__HR_FLAG.csv")
write.csv(ae_predict,file="HOVON_TRAIN_model_Accuracy.csv")


#save model seperately
saveRDS(svmfit,"./H_svmfit.rds")
saveRDS(rffit,"./H_rffit.rds")
saveRDS(nn_fit,"./H_nn_fit.rds")
saveRDS(lvq_fit,"./H_lvq_fit.rds")
saveRDS(gbm_fit,"./H_gbm_fit.rds")
saveRDS(glmfit,"./H_glmfit.rds")
saveRDS(fit_stacked,"./H_fit_stacked.rds")

H_genes<-colnames(Gene_expression)
write.csv(H_genes,"H_genes.csv")
